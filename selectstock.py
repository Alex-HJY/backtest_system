...
def preclose(stockcodestr,TRADE_DAYS):
    ...
    stockcell={'stockcode':stockcodestr,\
    'weight':2}
    return  stockcell
...
def selectstock(stockpool,changedt,factor):
    ...
    for i in range(0,len(changedt)):
        ...
        condition="S_INFO_WINDCODE='"+stockpool+"' and ((S_CON_INDATE<="+TRADE_DAYS[0][0]+" and S_CON_OUTDATE>="+TRADE_DAYS[0][0]+") or (S_CON_INDATE<="+TRADE_DAYS[0][0]+" and CUR_SIGN=1))"
        data='S_CON_WINDCODE'
        order='S_CON_WINDCODE'  
        stockcode=bf.gettingdata(data,tablename,condition,order)
